...
security_optional = HTTPBearer(auto_error=False)  # Optional auth (returns None if no token)
...
def get_current_user_optional(credentials: HTTPAuthorizationCredentials = Depends(security_optional), db: Session = Depends(get_db)) -> Optional[User]:
    """
