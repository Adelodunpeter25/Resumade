...
def score_experience(experience: list) -> Dict:
    ...
    complete_entries = sum(
        ...
        if exp.get("company")
        and exp.get("position")
        and exp.get("start_date")
    ...
    if complete_entries < len(experience):
        feedback.append(
            "Complete all experience entries (company, position, dates, description)"
        )
    ...
    action_verb_count = sum(
        1 for verb in ATS_KEYWORDS["action_verbs"] if verb in normalized
    )
    ...
    if action_verb_count >= 3:
        ...
        feedback.append(
            "Start bullet points with action verbs (Developed, Led, Managed)"
        )

    # Check for quantifiable achievements
    achievement_count = sum(
        detect_quantifiable_achievements(
    ...
    if achievement_count >= 3:
        ...
        score += 4
        feedback.append(
            "Add more quantifiable achievements (e.g., 'Increased sales by 40%')"
        )
    else:
...
def score_education(education: list) -> Dict:
    ...
    complete = all(
        edu.get("institution") and edu.get("degree") and edu.get("field_of_study")
    ...
    if complete:
        ...
        feedback.append(
            "Complete education entries (institution, degree, field of study)"
        )
...
def score_skills(skills: list, job_description: str = None) -> Dict:
    ...
    if job_description and skills:
        ...
        matches = sum(
            1 for kw in jd_keywords if any(kw in skill for skill in skill_names)
        )

        if matches < 3:
            feedback.append(
                f"Add skills from job description: {', '.join(jd_keywords[:5])}"
            )
