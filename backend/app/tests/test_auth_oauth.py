...
def test_google_callback_new_user(mock_get_user_info, mock_exchange_token, client, db_session):
    ...
    response = client.get(f"/api/auth/google/callback?code=mock_code&state={state}", follow_redirects=False)
...
@patch('app.services.oauth_service.GoogleOAuthService.exchange_code_for_token')
@patch('app.services.oauth_service.GoogleOAuthService.get_user_info')
def test_google_callback_existing_user(mock_get_user_info, mock_exchange_token, client, db_session, valid_user):
    ...
    user = db_session.query(User).filter(User.email == valid_user["email"].lower()).first()
    assert user.oauth_provider == "google"
