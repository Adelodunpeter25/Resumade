...
def get_dashboard_stats(
    ...
    new_resumes_week = (
        db.query(func.count(Resume.id)).filter(Resume.created_at >= week_ago).scalar()
    )

    # Template usage
    template_usage = (
        db.query(Resume.template, func.count(Resume.id).label("count"))
    ...
    avg_ats_score = (
        ...
        .filter(Resume.ats_score.isnot(None))
        .scalar()
    )

    # Active share links
    active_shares = (
        db.query(func.count(ShareLink.id)).filter(ShareLink.is_active).scalar()
    )
    ...
    return APIResponse(
        ...
        data={
            ...
            "sharing": {"active_links": active_shares},
        },
    )
...
def list_all_users(
    ...
    return APIResponse(
        ...
        data={"users": users, "page": page, "size": size, "total": total},
    )
...
def list_all_resumes(
    ...
    return APIResponse(
        ...
        data={"resumes": resumes, "page": page, "size": size, "total": total},
    )
