...
def _get_resume_by_id(resume_id: int, db: Session) -> Resume:
    ...
    return (
        db.query(Resume)
        .options(joinedload(Resume.user), joinedload(Resume.progress))
...
@router.post(
    "/", response_model=APIResponse[ResumeSchema], status_code=status.HTTP_201_CREATED
)
...
def create_resume(
    ...
    logger.info(
        f"Resume created with ID: {db_resume.id}, ATS Score: {ats_result['percentage']}%"
    )
    return APIResponse(
        success=True, message=ResponseMessages.RESUME_CREATED, data=db_resume
    )
...
async def get_resume(
    ...
    background_tasks.add_task(
        lambda: setattr(resume, "views", resume.views + 1) or db.commit()
    )
...
async def export_resume(
    ...
    background_tasks.add_task(
        lambda: setattr(resume, "downloads", resume.downloads + 1) or db.commit()
    )
...
def get_resume_score(
    ...
    job_description: Optional[str] = Query(
        None, description="Job description for context-aware scoring"
    ),
    role_level: str = Query(
        "mid",
        regex="^(entry|mid|senior)$",
        description="Job level for dynamic weighting",
    ),
    db: Session = Depends(get_db),
    current_user: Optional[User] = Depends(get_current_user_optional),
):
    ...
    ats_result = ATSService.calculate_ats_score(
        resume_dict, job_description, role_level
    )
...
async def parse_pdf_resume(request: Request, file: UploadFile = File(...)):
    ...
    if len(content) > FileConstants.MAX_PDF_SIZE_BYTES:
        raise HTTPException(
            status_code=400,
            detail=f"File size exceeds {FileConstants.MAX_PDF_SIZE_MB}MB limit",
        )
    ...
    try:
        ...
        return APIResponse(
            success=True, message="PDF parsed successfully", data=extracted_data
        )
...
def list_templates():
    ...
    return APIResponse(
        ...
        data={"categories": categorized, "all_templates": templates},
    )
...
def preview_resume_by_id(resume_id: str, db: Session = Depends(get_db)):
    """Render HTML preview of a specific resume"""
    # Handle guest mode
    if resume_id == "new":
        raise HTTPException(
            status_code=400,
            detail="Cannot preview unsaved guest resume. Please save first.",
        )
...
# Version History
@router.get(
    "/{resume_id}/versions", response_model=APIResponse[list[ResumeVersionSchema]]
)
def get_resume_versions(
    ...
    return APIResponse(
        ...
        data=[ResumeVersionSchema.from_orm(v) for v in versions],
    )
...
def create_resume_version(
    ...
    return APIResponse(
        ...
        data=ResumeVersionSchema.from_orm(version),
    )
...
def get_share_links(
    ...
    share_links = (
        db.query(ShareLink)
        .filter(ShareLink.resume_id == resume_id, ShareLink.is_active)
        .all()
    )
    ...
    for link in share_links:
        ...
        views = (
            db.query(func.count(ResumeAnalytics.id))
            .filter(
...
def create_share_link(
    ...
    if existing_link:
        expires_at = (
            existing_link.expires_at
            if existing_link.expires_at.tzinfo
            else existing_link.expires_at.replace(tzinfo=timezone.utc)
        )
        if expires_at > now:
            raise HTTPException(
                status_code=400, detail="Active link found for this resume"
            )
        else:
...
def preview_shared_resume(slug: str, db: Session = Depends(get_db)):
    ...
    share_link = (
        db.query(ShareLink).filter(ShareLink.slug == slug, ShareLink.is_active).first()
    )
    if not share_link:
        share_link = (
            db.query(ShareLink)
            .filter(ShareLink.token == slug, ShareLink.is_active)
            .first()
        )
    ...
    if share_link.expires_at:
        ...
        expires_at = (
            ...
            if share_link.expires_at.tzinfo
            else share_link.expires_at.replace(tzinfo=timezone.utc)
        )
        if expires_at < now:
...
def get_shared_resume(slug: str, request: Request, db: Session = Depends(get_db)):
    ...
    return APIResponse(
        success=True, message="Resume retrieved", data=ResumeSchema.from_orm(resume)
    )
...
def delete_share_link(
    ...
    share_link = (
        db.query(ShareLink)
        .filter(ShareLink.token == token, ShareLink.resume_id == resume_id)
        .first()
    )
    if not share_link:
        raise HTTPException(status_code=404, detail="Share link not found")
