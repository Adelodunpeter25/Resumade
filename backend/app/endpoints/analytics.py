...
def get_resume_analytics(
    ...
    views_by_day = (
        ...
        .group_by(func.date(ResumeAnalytics.created_at))
        .order_by(desc("date"))
        .all()
    )
    ...
    # Recent activity
    recent_activity = (
        db.query(ResumeAnalytics)
        .filter(
            ResumeAnalytics.resume_id == resume_id,
            ResumeAnalytics.created_at >= start_date,
        )
        .order_by(desc(ResumeAnalytics.created_at))
        .limit(50)
        .all()
    )
    ...
    return APIResponse(
        ...
        data={
            ...
            "views_by_day": [
                {"date": str(v.date), "count": v.count} for v in views_by_day
            ],
            "recent_activity": [
                {
...
def get_dashboard_analytics(
    ...
    resume_ids = [
        r.id
        for r in db.query(Resume.id).filter(Resume.user_id == current_user.id).all()
    ]

    if not resume_ids:
        return APIResponse(
            success=True,
            data={"total_views": 0, "total_downloads": 0, "top_resumes": []},
        )
    ...
    top_resumes = (
        db.query(Resume.id, Resume.title, func.count(ResumeAnalytics.id).label("views"))
        .join(ResumeAnalytics, Resume.id == ResumeAnalytics.resume_id)
        .filter(
            Resume.user_id == current_user.id,
            ResumeAnalytics.event_type == "view",
            ResumeAnalytics.created_at >= start_date,
        )
        .group_by(Resume.id, Resume.title)
        .order_by(desc("views"))
        .limit(5)
        .all()
    )
    ...
    return APIResponse(
        ...
        data={
            ...
            "total_downloads": total_downloads or 0,
            "top_resumes": [
                {"id": r.id, "title": r.title, "views": r.views} for r in top_resumes
            ],
        },
    )
