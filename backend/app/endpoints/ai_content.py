...
async def generate_bullet_points(
    ...
    if not result["success"]:
        raise HTTPException(
            status_code=503, detail=result.get("error", "AI service unavailable")
        )

    return APIResponse(
        success=True,
        message=f"Generated {result['count']} bullet point suggestions",
        data={"suggestions": result["suggestions"], "count": result["count"]},
    )
...
async def improve_description(
    ...
    result = ai_service.improve_description(
        current_text=data.current_text, context=context
    )
