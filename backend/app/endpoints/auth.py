...
@router.post(
    "/signup", response_model=APIResponse[Token], status_code=status.HTTP_201_CREATED
)
...
def signup(request: Request, user: UserCreate, db: Session = Depends(get_db)):
        ...
        raise HTTPException(
            ...
            detail=ErrorResponse(
                ...
                errors=[
                    ErrorDetail(
                        field="password",
                        message=f"Password must be at least {ValidationConfig.MIN_PASSWORD_LENGTH} characters",
                    )
                ],
            ).dict(),
        )
    ...
    full_name = InputValidator.sanitize_text(
        user.full_name, ValidationConfig.MAX_NAME_LENGTH
    )
        ...
        raise HTTPException(
            ...
            detail=ErrorResponse(
                ...
                errors=[
                    ErrorDetail(
                        field="full_name",
                        message=f"Name must be at least {ValidationConfig.MIN_NAME_LENGTH} characters",
                    )
                ],
            ).dict(),
        )
...
def login(request: Request, credentials: LoginRequest, db: Session = Depends(get_db)):
    ...
    if not user:
        ...
        raise HTTPException(
            status_code=401,
            detail=ErrorResponse(message="Invalid email or password").dict(),
        )
    ...
    if not user.hashed_password:
        ...
        raise HTTPException(
            status_code=401,
            detail=ErrorResponse(message="Please login with Google OAuth").dict(),
        )
...
def get_current_user_info(current_user: User = Depends(get_current_user)):
    ...
    return APIResponse(
        success=True, message="User information retrieved", data=current_user
    )
...
def forgot_password(
    ...
    return APIResponse(
        success=True, message="If email exists, reset link sent", data=None
    )
...
def reset_password(request: ResetPasswordRequest, db: Session = Depends(get_db)):
    ...
    if len(request.new_password) < ValidationConfig.MIN_PASSWORD_LENGTH:
        raise HTTPException(
            status_code=400,
            detail=f"Password must be at least {ValidationConfig.MIN_PASSWORD_LENGTH} characters",
        )
